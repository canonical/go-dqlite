...
// 4. Transfer the RecoveryKernel to each remaining node.
...
func belongsInKernel(name string) bool {
	return belongsInKernelPattern.MatchString(name)
}
...
func PrepareRecovery(dir string, address string, cluster []dqlite.NodeInfo) (*RecoveryKernel, error) {
	...
	if err := fileMarshal(dir, filepath.Join(dir, infoFile), me); err != nil {
	...
	for _, file := range files {
		if file.IsDir() || !belongsInKernel(file.Name()) {
			continue
		}
	...
	return &RecoveryKernel{tarball, cluster}, nil
}
...
func (kern *RecoveryKernel) Propagate(dir string, address string) error {
	...
	tr := tar.NewReader(&kern.Opaque)
	var hdr *tar.Header
	for hdr, err = nil, nil; err == nil; hdr, err = tr.Next() {
